...
int main(int argc, char* argv[]) {
  AB<Nif>* tree;
  int tree_type;
  std::cout << "Introduzca el tipo de árbol que desea crear (1: ABB, 2: ABE, 3: AVL): ";
  std::cin >> tree_type;
  if (tree_type == 1) {
    tree = new ABB<Nif>();
  } else if (tree_type == 2) {
    ...
    tree = new AVL<Nif>();
    std::cout << "¿Desea activar el modo traza? (1: sí, 0: no): ";
    int trace_mode;
    std::cin >> trace_mode;
    if (trace_mode == 1) {
      dynamic_cast<AVL<Nif>*>(tree)->SetTraceMode(true);
    }
  } else {
